...
public:
  ONNXModel(const char *path, float *output, size_t output_size, int runtime, std::string _file_name);
	~ONNXModel();
...
class ExtraModel : public RunModel {
public:
  ExtraModel(const char *path, float *output, size_t output_size, int runtime, std::string _file_name);
	~ExtraModel();
  void addRecurrent(float *state, int state_size);
  void addDesire(float *state, int state_size);
  void addTrafficConvention(float *state, int state_size);
