...
ONNXModel::ONNXModel(const char *path, float *_output, size_t _output_size, int runtime, std::string _file_name) {
  LOGD("loading model %s", path);
  //LOGW("loading extra model %s", path);
  ...
  std::string onnx_runner = exe_dir + _file_name;

  proc_pid = fork();
  if (proc_pid == 0) {
    //LOGD("spawning onnx process %s", onnx_runner.c_str());
    //LOGW("spawning onnx process %s \npipein_fd : %d\n pipeout_fd : %d", onnx_runner.c_str(),pipein[0],pipeout[1]);
    // dup2(pipein[0], 9);
    // dup2(pipeout[1], 12);
    std::string str_pipein = std::to_string(pipein[0]);
    std::string str_pipeout = std::to_string(pipeout[1]);
    char *argv[] = {(char*)onnx_runner.c_str(), (char*)path, (char*)str_pipein.c_str(), (char*)str_pipeout.c_str(), nullptr};

    //Close pipe may lead to openpilot unavilable int 2022.01.25, solved by canceled dup2 func and add para transmission
    close(pipein[1]);
    close(pipeout[0]);
...
ExtraModel::ExtraModel(const char *path, float *_output, size_t _output_size, int runtime, std::string _file_name) {
  //LOGD("loading model %s", path);
  LOGW("loading extra model %s", path);
  output = _output;
  output_size = _output_size;
  ...
  std::string exe_dir = util::dir_name(util::readlink("/proc/self/exe"));
  std::string onnx_runner = exe_dir + "/runners/onnx_runner_extra.py";

  proc_pid = fork();
  if (proc_pid == 0) {
    //LOGD("spawning onnx process %s", onnx_runner.c_str());

    //The LOGW log leads to openpilot unavilable in 2022.01.25
    //LOGW("spawning onnx extra process %s \nextra_pipein_fd : %d\n extra_pipeout_fd : %d", onnx_runner.c_str(),pipein[0],pipeout[1]);
...
void ExtraModel::pwrite(float *buf, int size) {
  char *cbuf = (char *)buf;
  int tw = size*sizeof(float);
  while (tw > 0) {
    int err = write(pipein[1], cbuf, tw);
...
void ExtraModel::pread(float *buf, int size) {
  char *cbuf = (char *)buf;
  int tr = size*sizeof(float);
  struct pollfd fds[1];
  fds[0].fd = pipeout[0];
...
void ExtraModel::addRecurrent(float *state, int state_size) {
  rnn_input_buf = state;
  rnn_state_size = state_size;
}

void ExtraModel::addDesire(float *state, int state_size) {
  desire_input_buf = state;
  desire_state_size = state_size;
}

void ExtraModel::addTrafficConvention(float *state, int state_size) {
  traffic_convention_input_buf = state;
  traffic_convention_size = state_size;
}

void ExtraModel::execute(float *net_input_buf, int buf_size) {
  // order must be this
  pwrite(net_input_buf, buf_size);
  if (desire_input_buf != NULL) {
    pwrite(desire_input_buf, desire_state_size);
